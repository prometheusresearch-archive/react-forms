  ...
  describe('Lens', () => {
    ...
    describe('mod()', () => {
      ...
      describe('creating values', () => {
        ...
        it('can create nested values', () => {
          var lens = Lens.make({});
          assert.deepEqual(
            Lens.make({}).get('a', {}).get('b', []).get(0).mod(42).root(),
            {a: {b: [42]}});
        });
      });
    ...
    describe('for()', () => {
      ...
      it('allows creating a lens for a different value with the same path', () => {
        var data1 = {a: 1};
        var data2 = {a: 2};
        var lens1 = Lens.make(data1).get('a');
    ...
    describe('reduce()', () => {
      ...
      it('reduces arrays', () => {
        assert.equal(Lens.make([1, 2, 3]).reduce((a, b) => a + b), 6);
        assert.equal(Lens.make([1, 2, 3]).reduce((a, b) => a + b, 1), 7);
        assert.equal(Lens.make([1]).reduce((a, b) => a + b, 0), 1);
        assert.equal(Lens.make([]).reduce((a, b) => a + b, 0), 0);
        assert.throws(() => Lens.make([]).reduce((a, b) => a + b));
      });

      it('reduces objects', () => {
        assert.equal(Lens.make({a: 1, b: 2, c: 3}).reduce((a, b) => a + b), 6);
        assert.equal(Lens.make({a: 1, b: 2, c: 3}).reduce((a, b) => a + b, 1), 7);
        assert.equal(Lens.make([1]).reduce((a, b) => a + b, 0), 1);
        assert.equal(Lens.make([]).reduce((a, b) => a + b, 0), 0);
        assert.throws(() => Lens.make([]).reduce((a, b) => a + b));
      });
      ...
      it('reduces scalars', () => {
        assert.equal(Lens.make(1).reduce((a, b) => a + b, 0), 1);
        assert.throws(() => Lens.make(1).reduce((a, b) => a + b));
      });

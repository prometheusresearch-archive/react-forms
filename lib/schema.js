...
exists.toString = function() {
  return '<Validate value exists>';
};

exists.validateNull = true;
...
function isSchema(node) {
  return isMapping(node) || isList(node) || isScalar(node);
}
...
function Scalar(props) {
  ...
  return new ScalarNode({
    props: props,
    name: props.get('name'),
    validate: [exists, props.get('validate', emptyFunction.thatReturnsArgument)],
...
function Mapping(...args) {
  var [props, ...children] = args;
  if (isSchema(props) || Array.isArray(props)) {
    children = args;
    props = {};
  }
  var childrenMap = {};
  if (children.length !== 0) {
    forEachNested(children, (child) => {
      if (child) {
        invariant(
          child.name,
  ...
  return new MappingNode({
    ...
    defaultValue: props.get('defaultValue', OrderedMap.empty()),
...
function List(...args) {
  ...
  return new ListNode({
    ...
    defaultValue: props.get('defaultValue', Vector.empty()),
    children
  });
}

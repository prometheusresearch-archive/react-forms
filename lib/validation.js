...
function serialize(node, value) {
  ...
  if (schema.isScalar(node)) {
    result = getTypeFromSchema(node).serialize(value);
  } else if (schema.isMapping(node)) {
    result = {};
    for (var k in value) {
      if (node.children[k]) {
...
function validate(node, value) {
  if (schema.isMapping(node)) {
    return validateMapping(node, value);
  } else if (schema.isList(node)) {
    return validateList(node, value);
  } else if (schema.isScalar(node)) {
    return validateScalar(node, value);
  } else {
    invariant(
      false,
...
function validateOnly(node, value, children) {
  if (schema.isMapping(node)) {
    return validateMappingOnly(node, value, children);
  } else if (schema.isList(node)) {
...
function validateMapping(node, value) {
  var childrenValidation = validateMappingChildren(node, value);
  ...
  var validation = validateMappingOnly(
      node,
      convertedValue,
...
function validateMappingOnly(node, value, children) {
...
function validateMappingChildren(node, value) {
  var validation = {};
  var children = {};
...
function validateScalar(node, value) {

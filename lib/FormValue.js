...
class Value {
  ...
  parent() {
    ...
    var parentSchema = this.schemaLens.parent();
    var parentSchemaNode = parentSchema.val();

    if (!s.isSchema(parentSchemaNode) && !s.isList(parentSchemaNode)) {
      parentSchema = parentSchema.parent();
    }

    return _make(
      parentSchema,
      this.valueLens.parent(),
      this.serializedValueLens.parent(),
      this.validationLens.parent().parent(),
      path
    );
  }
...
class PropetyValue extends Value {
  ...
  updateSerializedValue(serializedValue) {
    ...
    var valueLens = this.valueLens.mod(validation.value);
...
class ListValue extends CompositeValue {
  ...
  add(value) {
    ...
    var localValidation = v.validateOnly(
      this.schema,
      this.value.concat(value),
      this.validationLens.val().children
    );
    var valueLens = this.valueLens.mod(localValidation.value);

    return new this.constructor(
      this.schemaLens,
      valueLens,
      this.serializedValueLens.mod(this.serializedValue.concat(serializedValue)),
      this.validationLens.mod(localValidation.validation),
      this.path
    );
  }

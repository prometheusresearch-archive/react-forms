...
class Value {
  ...
  update(newValue) {
    ...
    return new this.constructor(
      this._attributes.merge({value, serialized, validation}),
      this.path
    );
  }

  with(attrs) {
    return new this.constructor(this._attributes.merge(attrs), this.path);
  }
...
function make(schema, value, externalValidation, onUpdate) {
  externalValidation = externalValidation || ValidationResult.success;
  var params = _make({schema, value, externalValidation});
  var ctor = _getCtor(schema);
  return new ctor(new ValueAttributes(merge(params, {onUpdate})), []);
}

...
class Value {
  ...
  constructor(
      parent, name, path, schema, value, serialized,
    ...
    this.externalValidation = externalValidation || v.success;
  ...
  forSchema(schema) {
    ...
    var newRoot = make(schema, root.value, root.serialized, undefined, root.externalValidation);
  ...
  get(name) {
    return _make(
      ...
      (this.externalValidation.children && this.externalValidation.children[name]) || v.success
    );
  }
...
function _make(parent, name, path, schema, value, serialized, validation, externalValidation) {
  var constructor;
  ...
  return new constructor(
    parent, name, path, schema,
    value, serialized, validation, externalValidation);
}

function make(schema, value, serialized, validation, externalValidation) {
  return _make(null, null, [], schema, value, serialized, validation, externalValidation);
}

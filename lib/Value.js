...
class Value {
  ...
  update(newValue) {
    ...
    for (var i = 0, len = trace.length; i < len; i++) {
      ...
      if (i === 0) {
        ...
        var onUpdate = schema.props.get('onUpdate');
        if (onUpdate) {
          newValue = onUpdate(newValue);
        }
  ...
  onChildUpdate(key, {value, validation, serialized}) {
    ...
    var onUpdate = this.schema.props.get('onUpdate');
    if (onUpdate) {
...
Object.defineProperty(Value.prototype, 'schema', {
  get: function() {
    if (this._schema === null) {
      this._schema = this._attributes.schema.instantiate(this._attributes.schema);
    }
    return this._schema;
  }
});
...
class ListValue extends Value {
  ...
  push(value) {
    ...
    return this.child(newIndex).update(value).parent;
  }
...
function _makeList(args, options) {
  ...
  value.forEach((value, name) => {
    var child = makeAttributes({
      schema: schema.child(name).instantiate(value),
      value
    }, {strip: options.strip});
    if (child === undefined) {
      return;

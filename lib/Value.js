...
class Value {
  ...
  _updateSelf(update) {
    u.invariant(
      !(update.value === undefined && update.serialized === undefined)
    );

    if (update.value === undefined || update.validation === undefined) {
      var toValidate = update.value !== undefined ? update.value : update.serialized;
      var validated = v.validate(this.schema, toValidate);
      update.value = validated.value;
      update.validation = validated.validation;
    }
...
function _make(parent, name, path, schema, value, serialized, validation) {
  ...
  return new constructor(parent, name, path, schema, value, serialized, validation);
}

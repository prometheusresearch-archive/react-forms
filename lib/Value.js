...
var {ScalarNode, MappingNode,
    ListNode, CompositeNode}  = require('./schema');
...
class Value {
  ...
  setSerialized(serialized: String) {
    ...
    var attributes = {value, serialized, dirty: DIRTY};
  ...
  setExternalValidation(externalValidation: ValidationResult) {
    return this.__update({externalValidation});
  }

  setSchema(abstractNode: Node) {
    var node = abstractNode.instantiate(this.value);
    return this.__update({node, abstractNode});
  }
  ...
  static create(abstractNode: Node, value: Any, onUpdate: Function, root: Function): Value {
 ...
 */
function validate(node: Node, value: Any, childrenValidation): ValidationResult {

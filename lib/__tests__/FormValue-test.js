...
describe('FormValue', () => {
  ...
  describe('getting parent value', () => {

    it('allows getting a parent value of Property inside Schema-typed value', () => {
      var schema = <Schema><Property name="name" /></Schema>;
      var value = FormValue(schema, {name: 'name'});
      assert.deepEqual(value.get('name').parent, value);
    });

    it('allows getting a parent value of value inside List-typed value', () => {
      var schema = <List><Property /></List>;
      var value = FormValue(schema, ['name']);
      assert.deepEqual(value.get(0).parent, value);
    });

    it('returns null in case there is no parent value', () => {
      var schema = <List><Property /></List>;
      var value = FormValue(schema, ['name']);

...
describe('forms', () => {
  ...
  describe('lens', () => {
    ...
    describe('creating lenses', () => {
      ...
      it('returns an argument if it is a lens already', () => {
        var data = 42;
        var lens = make(data);
        var lens2 = make(lens);
        assert.strictEqual(lens, lens2);
        assertLensPointsTo(lens, data);
        assertLensPointsTo(lens2, data);
      });
    });
    ...
    describe('isUndefined()', () => {
      ...
      it('checks if a value lens pointed to exist', () => {
        assert.ok(make({}).get('a').isUndefined());
        assert.ok(make({}).get('a', {}).get('b').isUndefined());
        assert.ok(make({}).get('a').get('b').isUndefined());
        assert.ok(make(undefined).isUndefined());
      });
    });

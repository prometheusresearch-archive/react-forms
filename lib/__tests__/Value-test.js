...
var {Mapping, List, Scalar,
    isScalar, isList, isMapping,
    Dynamic} = require('../schema');
...
describe('Value', function() {
  ...
  describe('creating new values', function() {
    ...
    it('creates new values with dynamic schema', function() {
      var schema = Mapping({
        dynamic: Dynamic(function DynamicMapping(value) {
  ...
  describe('onUpdate callbacks', function() {

    var schema = Mapping({
      onUpdate(value) {
        var l = value.get('l');
        if (l && l.length === 0) {
          return value.remove('l');
        } else {
          return value;
        }
      }
    ...
    it('fires on update to a child', function() {
      var value = Value(schema);
      value = value.child('l').push(1).root;
      assert.ok(value.value.equals(OrderedMap({l: Vector(1)})));
      value = value.child('l').splice(0, 1);
      assert.ok(value.value.equals(OrderedMap({})));
    });

    it('fires on update to self', function() {
      var value = Value(schema, {l: [1]});
      assert.ok(value.value.equals(OrderedMap({l: Vector(1)})));
      value = value.child('l').splice(0, 1);
      assert.ok(value.value.equals(OrderedMap({})));
    });

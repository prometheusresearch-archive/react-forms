...
describe('Value:', function() {
  ...
  describe('Value.create(schema, initial):', function() {
    ...
    function itGrows(schema, initial, expected) {
      ...
      var grown = Value.create(schema, initial);
      assert.ok(
  ...
  describe('Value.prototype.get(key):', function() {
    ...
    function itGets(schema, key, initial, expected) {
      ...
      var value = Value.create(schema, initial);
  ...
  describe('Value.prototype.keys():', function() {
    ...
    it('returns an iterable of keys', function() {
      ...
      var value = Value.create(schema, {x: [1, 2, 3], y: 2, z: 3});
  ...
  describe('Value.prototype.set(value):', function() {
    ...
    it('updates nested mapping', function() {
      var schema = Mapping({
        x: Mapping({y: Scalar({type: 'number', defaultValue: 1})})
      });
    ...
    it('updates nested mapping', function() {
      ...
      itUpdates(
        ...
        {
          value: {x: [{y: 1}]},
          serialized: Map({x: Immutable.List([Map({y: 1})])}),
          validation: ValidationResult.success()
        },
      ...
      itUpdates(
        ...
        {
          value: {x: [{y: 3}]},
          serialized: Map({x: Immutable.List([Map({y: 3})])}),
          validation: ValidationResult.success()
        },
  ...
  describe('Value.prototype.notify()', function() {
    var schema = Mapping({
      x: Mapping({y: Scalar()}),
      v: Mapping({u: Scalar()})
    });
    var onUpdate = sinon.spy();
    var value = Value.create(schema, undefined, onUpdate);

    value.notify();
    assert.ok(onUpdate.callCount, 1);
    value.notify();
    assert.ok(onUpdate.callCount, 2);
  });
...
describe('Value with recursive schema:', function() {
  ...
  it('works', function() {
    var value = Value.create(treeNode);
...
describe('Value.prototype.validation', function() {
  ...
  describe('get', function() {
    ...
    it('validates mappings with required keys', function() {
      ...
      value = Value.create(schema);
      assertNotValid(value);
      assertNotValid(value.get('x'));
      assertValid(value.get('y'));

      value = value.get('y').set('x').root;
      assertNotValid(value);
      assertNotValid(value.get('x'));
      assertNotValid(value.get('y'));

      value = value.get('x').set('zzz').root;
      assertNotValid(value);
      assertValid(value.get('x'));
      assertNotValid(value.get('y'));

      value = value.get('y').set(123).root;
      assertValid(value);
      assertValid(value.get('x'));
      assertValid(value.get('y'));

      value = value.get('x').set(undefined).root;
      assertNotValid(value);
      assertNotValid(value.get('x'));
      assertValid(value.get('y'));
    });

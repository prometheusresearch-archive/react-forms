...
describe('forms', () => {
  ...
  describe('FormMixin', () => {
    ...
    function makeValue(value) {
      return Value({schema: form.props.schema, value});
    }

    function renderForm(props) {
      renderedCount = 0;
      props = merge({
        schema: <Scalar validate={(v) => v > 0} />
      }, props);
    ...
    describe('value', () => {
      ...
      it('uses default value from schema if not valid value was passed', () => {
        renderForm({schema: <Scalar defaultValue="125" />});
    ...
    describe('externalValidation', () => {
      ...
      it('has externalValidation as valid by default', () => {
        renderForm();
        assert.ok(validation.isSuccess(form.value().externalValidation));
      });
      ...
      it('allows passing external validation via prop', () => {
        ...
        assert.ok(validation.isFailure(form.value().externalValidation));
        assert.deepEqual(form.value().externalValidation, externalValidation);
      });
